...
import { Loader2, Wallet, MapPin, User, Barcode, Shield, Zap, CheckCircle, ArrowRight, Gem, Star, Award, Leaf } from "lucide-react";
...
const t = {
  es: {
    ...
    formDesc: "Completa todos los campos para registrar tu lote en la blockchain",
    batchId: "ID del Lote",
    batchIdPlaceholder: "Ej: MG-2024-001",
    ...
    successDesc: "Tu lote ha sido registrado en la blockchain",
    registeredVar: "Variedad Registrada",
    trackBatch: "Rastrear Lote",
    registerAnother: "Registrar Otro",
    note: "Nota",
    noteMsg: "Al registrar, una transacción será simulada en Polygon Amoy Testnet. Asegúrate de tener MATIC de prueba.",
    network: "Red",
    fillAll: "Completa todos los campos",
    selectVar: "Selecciona una variedad",
    errorReg: "Error al registrar el lote",
    savedDb: "✓ Guardado en BD",
  },
  en: {
    title: "Register Batch",
    subtitle: "Register your mango batch on the blockchain",
    formTitle: "Registration Form",
    formDesc: "Complete all fields to register your batch on the blockchain",
    batchId: "Batch ID",
    batchIdPlaceholder: "Ex: MG-2024-001",
    ...
    successDesc: "Your batch has been registered on the blockchain",
    registeredVar: "Registered Variety",
    trackBatch: "Track Batch",
    registerAnother: "Register Another",
    note: "Note",
    noteMsg: "A transaction will be simulated on Polygon Amoy Testnet. Make sure you have test MATIC.",
    network: "Network",
...
const Registrar = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const batchData: BatchRecord = {
        ...
        metadata: { varietyId: formData.variedad, timestamp: new Date().toISOString(), network: "Polygon Amoy", emoji: varietyInfo.emoji },
      };
      ...
      if (result.success) {
        ...
        lotes.push({ loteId: formData.loteId, productor: formData.productor, ubicacion: formData.ubicacion, variedad: formData.variedad, varietyName: varietyInfo.name, calidad: formData.calidad, hash: mockHash, timestamp: new Date().toISOString(), network: "Polygon Amoy" });
  ...
  const handleReset = () => {
    ...
    setFormData({ loteId: "", productor: "", ubicacion: "Piura", variedad: "", calidad: "" });
  };
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input placeholder={i.batchIdPlaceholder} value={formData.loteId}
                      ...
                      className="rounded-xl border-border bg-background h-12 text-base focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all" required />
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={formData.ubicacion} onValueChange={(v) => setFormData({ ...formData, ubicacion: v })}>
                        <SelectTrigger className="rounded-xl border-border bg-background h-12 text-base"><SelectValue /></SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={formData.calidad} onValueChange={(v) => setFormData({ ...formData, calidad: v })}>
                        <SelectTrigger className="rounded-xl border-border bg-background h-12 text-base"><SelectValue placeholder={i.qualityPlaceholder} /></SelectTrigger>
                        <SelectContent className="rounded-xl border-border">
                          ...
                          <SelectItem value="Export"><div className="flex items-center gap-2"><Gem className="h-4 w-4 text-secondary" />{i.export}</div></SelectItem>
                          <SelectItem value="First Grade"><div className="flex items-center gap-2"><Award className="h-4 w-4 text-primary" />{i.first}</div></SelectItem>
                          <SelectItem value="Second Grade"><div className="flex items-center gap-2"><Award className="h-4 w-4 text-muted-foreground" />{i.second}</div></SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={formData.variedad} onValueChange={(v) => setFormData({ ...formData, variedad: v })}>
                      <SelectTrigger className="rounded-xl border-border bg-background h-12 text-base"><SelectValue placeholder={i.varietyPlaceholder} /></SelectTrigger>
